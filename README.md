...
| 文件 | 内容 |
|---|---|
| `SKILL.md` | 取证、论证、文字、复核四节，外加材料仓库维护 |
| `evidence.md` | 来源层级表、声誉断言的四道门、版本与重查、标注粒度 |

`evidence.md` 是按需加载的：只在真正要给材料定级、或要下一条涉及项目声誉的断言时才会被读进上下文。

四节里最该先看的是「文字」和「复核」。文字那节嵌了一段真实的目标声音样本，比任何禁用清单都管用；复核是一张分级清单，写完单独走一遍，前四级（读者、结构、事实、论证）发现的问题必须改，后两级（句子、术语）在与事实冲突时让步。
